...
namespace bite
{
	namespace algorithm
	{
		...
		inline constexpr _T greatest_common_divisor(_T a, _T b)
		{
			static_assert(std::is_integral_v<_T>, "_T should be an integral type");
			_T tmp{};
			while (b)
			{
				tmp = b;
		...
		inline constexpr uint32_t crc32(const uint8_t data[], size_t len)
		{
		...
		inline constexpr void strassen_mul(const float a[4], const float b[4], float res[4])
		{
			const float m[]
			{
				(a[ 0 ] + a[ 3 ]) * (b[ 0 ] + b[ 3 ]),
			...
			res[ 0 ] = m[ 0 ] + m[ 3 ] - m[ 4 ] + ((a[ 1 ] - a[ 3 ]) * (b[ 2 ] + b[ 3 ]));
			res[ 1 ] = m[ 2 ] + m[ 4 ];
			res[ 2 ] = m[ 1 ] + m[ 3 ];
			res[ 3 ] = m[ 0 ] - m[ 1 ] + m[ 2 ] + ((a[ 2 ] - a[ 0 ]) * (b[ 0 ] + b[ 1 ]));
		}

		inline constexpr std::array<float, 4> strassen(const float a[ 4 ], const float b[ 4 ])
		{
			float res[ 4 ]{};
			strassen_mul(a, b, res);
